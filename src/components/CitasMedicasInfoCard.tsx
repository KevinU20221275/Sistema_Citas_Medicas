...
export function CitaMedicaInfoCard({estadoCita, numeroCitas}:{estadoCita:string, numeroCitas: number}){
    ...
    return (
        <div 
        className={`bg-white flex flex-col card items-center justify-center p-3 rounded-md border-[1px] 
        ${estadoCita === EstadoConsulta.Cancelada ? 'border-red-600' :
        estadoCita === EstadoConsulta.Completada ? 'border-green-600' : 'border-zinc-500'}`} >
            ...
            estadoCita === EstadoConsulta.Completada ? 'bg-green-600 text-white' : 'text-white bg-zinc-500'}`}
            >
                {numeroCitas ?? 0}
            </div>
        </div>
